...
/*   Updated: 2023/11/15 23:26:53 by otamrani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int calculate_half_of_rayangle(t_data *data)
{
    if((data->ray_angle >= 0 && data->ray_angle <= M_PI))
        return(1);
    return(2);
}
int check_distence_D(t_data *data)
{
    int x = data->dx / TILE;
    int y = data->dy / TILE;
...
void intercept_v(t_data *data)
{
    ...
    while (data->y_intercept >= 0 && data->y_intercept < TILE * data->all->y_of_map&& data->x_intercept >= 0 && data->x_intercept < TILE * data->all->x_of_map) {
    int x = data->x_intercept / TILE;
    int y = data->y_intercept / TILE;
    if (y < data->all->y_of_map && x < data->all->x_of_map && (data->all->map[y][x] == '1' || data->all->map[y][x] == 'D')) {
...
void line_door(t_data *data)
{
    ...
    if(data->all->map[y][x] == 'D')
    {
        ...
        if ((data->all->map[y / TILE][x / TILE] != 'T') && (data->all->map[(y + TILE) / TILE][x / TILE] != 'T') 
            && (data->all->map[y / TILE][(x + TILE) / TILE] != 'T') && (data->all->map[(y - TILE) / TILE][x / TILE] != 'T') 
            && (data->all->map[y / TILE][(x - TILE) / TILE] != 'T') //side
            && (data->all->map[(y + TILE) / TILE][(x + TILE) / TILE] != 'T') 
            && (data->all->map[(y + TILE) / TILE][(x - TILE) / TILE] != 'T') && (data->all->map[(y - TILE) / TILE][(x + TILE)/ TILE] != 'T') 
            && (data->all->map[(y - TILE) / TILE][(x - TILE) / TILE] != 'T')
            )
        {
            close_doors(data);
...
//   else
//   {
//     x = ((int)data->pos_x) / TILE;
//     y =  ((int)data->pos_y) / TILE;
//     printf("x = %d y = %d\n", x, y);
//     if((x != (data->xd / TILE) || data->yd != (data->dy / TILE)) && (data->xd != 0 && data->yd != 0))
//     {
//         data->all->map[data->yd][data->xd] = 'D';
//     }
//   }
//   data->check++;
//   if(data->check == 2)
...
int player_collision(t_data *data)
{
    ...
    if(data->all->map[(y + 5) / y1][(x + 5) / x1] == 'D')
            return(1);
    // if(data->xd != 0 && data->yd != 0 && data->pos_x / TILE != data->xd && data->pos_y / TILE != data->yd)
    //     data->all->map[data->yd][data->xd] = 'D';
